...
Workers must start fresh, exit when done, retain their selected model on steer,
and never recursively delegate. Never silently inherit an expensive model or
enable native subagents in workers. Keep existing user configuration intact.
